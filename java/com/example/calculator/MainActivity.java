...
public class MainActivity extends AppCompatActivity {
    ...
    public void onClickBtnEqual(View v) {
        ...
        if ((result % 1) == 0) {
            textView.setText(String.format(Locale.US, "%.0f", result));
            hasPoint = false;
        } else {
            textView.setText(format.format(result));
            hasPoint = true;
        }
    ...
    public void onClickBtnAC(View v) {
        textView.setText("0");
        textView2.setText("");
        oper = "";
        hasPoint = false;
    ...
    public void onClickBtnMult(View v) {
        if (oper.equals("×")) {
            first = Double.parseDouble(textView2.getText().toString().replace("×", ""));
            second = Double.parseDouble(textView.getText().toString());
            result = first * second;
            textView2.setText(String.format(Locale.US, "%f", result).concat("x"));
        }
        if (oper.equals("")) {
    ...
    public void onClickBtnDivide(View v) {
        if (oper.equals("/")) {
            ...
            second = Double.parseDouble(textView.getText().toString());
            result = first / second;
            textView2.setText(String.format(Locale.US, "%f", result).concat("/"));
        }
        if (oper.equals("")) {
            first = Double.parseDouble(textView.getText().toString());
    ...
    public void onClickBtnPercentage(View v) {
        if (!textView2.getText().toString().isEmpty()) {
            first = Double.parseDouble(textView2.getText().toString().replace(oper, ""));
            second = Double.parseDouble(textView.getText().toString());
            result = (first / 100) * second;
            textView2.setText(String.format(Locale.US, "%f", result));
        }
    }
